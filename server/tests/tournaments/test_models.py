...
def test_torunaments_model():
    tournament = Tournamnets(category='START', event_date="2022-02-04T09:00:00Z", place="Test place", prestige=100, surface='CLAY', capacity=16, price=1000)
    tournament.save()
    ...
    assert str(tournament) == f'Tournament of category {tournament.category} on {tournament.event_date} at {tournament.place}'